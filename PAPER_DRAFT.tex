...
% --- Layout hygiene: prevent float gaps and cramped tables ---
\usepackage{float}                          % float placement control
\usepackage{booktabs}                       % professional rules (toprule/midrule)
\usepackage[section]{placeins}              % keep floats inside their own section
\renewcommand{\arraystretch}{1.25}         % breathing room between table rows

% --- Even justification: kill the "ladder" look from long \texttt/DOI tokens ---
\usepackage{microtype}                      % protrusion + expansion = smooth line edges
\emergencystretch=2em                       % allow extra stretch instead of ugly gaps

% --- Uniform paragraphs: every paragraph flush-left, separated by space ---
\usepackage{parskip}                        % no alternating first-line indents

% --- Full-width abstract (override CEURART's inset side block) ---
\makeatletter
\AddToHook{env/abstract/begin}{\leftskip=0pt\relax}
\makeatother
...
\ead{mohdibadullah24@kmit.edu.in}
...
name of the candidate across four name-origin groups and two genders, we find a monotonic
...
skill and task extraction of the winning system reduced surface-lexical influence, but that
hypothesis was never tested.

We test it. We ask a narrower and falsifiable question: given a fixed CV, how much does the
name of the candidate alone move their rank, and where in the pipeline does that movement
happen?
...
\begin{table}[!htb]
\centering
\caption{Retrieval pipeline stages evaluated in this study.}
\label{tab:pipeline}
\small
...
\begin{table}[!htb]
\centering
\caption{Rank stability (RBO) under name-only perturbation, per stage and language.}
\label{tab:stage_rbo}
\begin{tabular}{lcc}
\toprule
...
\begin{table}[!htb]
\centering
\caption{Paired-bootstrap RBO gaps against BM25, with 95\% confidence intervals.}
\label{tab:bm25_gaps}
\small
\begin{tabular}{@{}lcc@{}}
\toprule
...
\begin{table}[!htb]
\centering
\caption{RBO improvement from identity interventions, per stage and language.}
\label{tab:intervention_rbo}
\small
\begin{tabular}{@{}lcccc@{}}
\toprule
 & \multicolumn{2}{c}{English} & \multicolumn{2}{c}{Spanish} \\
\cmidrule(lr){2-3} \cmidrule(l){4-5}
stage & change in RBO & 95\% CI & change in RBO & 95\% CI \\
\midrule
...
\begin{table}[!htb]
\centering
\caption{English average-precision effect of each intervention, against \texttt{none}.}
\label{tab:map_english}
\small
...
\begin{table}[!htb]
\centering
\caption{Spanish average-precision effect of each intervention, against \texttt{none}.}
\label{tab:map_spanish}
\small
\setlength{\tabcolsep}{4.5pt}
\begin{tabular}{@{}llcccc@{}}
\toprule
...
\begin{table}[!htb]
\centering
\caption{CPU median latency per query, per stage (English dev set).}
\label{tab:latency}
\begin{tabular}{lc}
\toprule
